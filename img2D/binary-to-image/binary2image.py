...
def save_file(filename, data, size, image_type, output_dir):
    try:
        ...
        imagename = os.path.join(output_dir, image_type, name + '_' + image_type + '.png')
...
def get_size(data_length, width=None):
    ...
    if width is None:  # width not specified
        size = data_length
...
def run(file_queue, output_dir, width, analysis_type):
    while not file_queue.empty():
        filename = file_queue.get()
        if analysis_type in ('L', 'both'):
            createGreyScaleImage(filename, output_dir, width)
        if analysis_type in ('RGB', 'both'):
            createRGBImage(filename, output_dir, width)
        file_queue.task_done()


def main(input_dir, output_dir, width=None, thread_number=7, analysis_type='both'):
    ...
    for index in range(thread_number):
        thread = Thread(target=run, args=(file_queue, output_dir, width, analysis_type))
        thread.daemon = True
        thread.start()
    file_queue.join()
...
if __name__ == '__main__':
    ...
    parser.add_argument('input_dir', help='Input directory path which includes executable files')
    parser.add_argument('output_dir', help='Output directory path to save images')
    parser.add_argument('--width', type=int, help='Width of the output image (optional)', default=None)
    parser.add_argument('--analysis_type', type=str, choices=['L', 'RGB', 'both'], help='Type of analysis: L for grayscale, RGB for color, both for both types', default='both')

    args = parser.parse_args()

    main(args.input_dir, args.output_dir, width=args.width, analysis_type=args.analysis_type)
