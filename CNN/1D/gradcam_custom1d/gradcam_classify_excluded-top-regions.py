...
if __name__ == "__main__":
    ...
    for apk, label in tqdm(apk_list, desc="Classifying"):
        y_true.append(label)
        ...
        if len(dataset) == 0:
            y_pred.append(-1)
            logger.warning(f"No samples for APK: {apk}")
            continue
        ...
        top_n = 5
        excluded_indices = set([i for i, _, _ in region_scores[:top_n]])
        ...
        logger.info(f"APK: {apk} | True: {label} | Pred (w/o top-{top_n} regions): {majority_pred} --> {result}")
