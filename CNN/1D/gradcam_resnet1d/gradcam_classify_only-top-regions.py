...
if __name__ == "__main__":
    ...
    logger.remove()
    logger.add("gradcam_top_regions_resnet1d.log", format="{message}")
    ...
    for apk_path in apk_list:
        ...
        dataset = CustomDataset(root_dir=root_dir, transform=transform, apk_list=[apk_path], class_filter=label, selection_mode="data_stack")
        ...
        top_indices = [idx for idx, _, _ in region_scores[:5]]
        ...
        for i in top_indices:
            ...
            input_tensor = img_tensor.unsqueeze(0).to(device)
            gradcam = GradCAM1D(model, model.feature_extractor[-1])
            heatmap = gradcam.generate(input_tensor, class_idx=label, return_map=True)
        ...
        logger.info(f"Predicted (only top pixels from top regions): {majority_pred} | True: {label} | Result: {result}")
