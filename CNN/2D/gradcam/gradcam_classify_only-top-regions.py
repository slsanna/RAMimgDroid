...
if __name__ == "__main__":
    ...
    log_path = os.path.join(f"gradcam_RGB_resnet18_True_only-top-regions.log")
    ...
    for apk_path in apk_list:
        ...
        # Step 1: Get Grad-CAM scores for each region
        ...
        # Step 2: Keep top-N most influential regions
        region_scores.sort(key=lambda x: x[2], reverse=True)
        top_n_to_include = 5
        included_indices = set([idx for idx, _, _ in region_scores[:top_n_to_include]])

        # Step 3: Run classification only on top regions
        region_predictions = []
        ...
        if region_predictions:
            ...
            majority_label = -1  # fallback if nothing included
        ...
        logger.info(f"Predicted (ONLY top-{top_n_to_include} regions): {majority_label} | True: {label} | Result: {result}")
