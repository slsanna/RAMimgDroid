...
class GradCAM:
    ...
    def generate(self, input_tensor, class_idx=None, return_map=False):
        ...
        return cam.cpu() if return_map else cam.mean().item()
...
def keep_only_top_pixels(image_tensor, heatmap, num_pixels=50):
    if isinstance(heatmap, np.ndarray):
        heatmap = torch.tensor(heatmap, device=image_tensor.device)
    else:
        heatmap = heatmap.to(image_tensor.device)

    heatmap_flat = heatmap.flatten()
    k = min(num_pixels, heatmap_flat.shape[0])
    if k == 0:
        return image_tensor * 0
    topk_indices = torch.topk(heatmap_flat, k).indices
    h, w = heatmap.shape
    rows, cols = np.unravel_index(topk_indices.cpu().numpy(), (h, w))
    mask = torch.zeros_like(heatmap, dtype=torch.bool)
    mask[rows, cols] = True
    new_img = torch.zeros_like(image_tensor)
    for c in range(image_tensor.shape[0]):
        new_img[c] = torch.where(mask, image_tensor[c], torch.tensor(0.0, device=image_tensor.device))
    return new_img
...
if __name__ == "__main__":
    ...
    for apk_path in apk_list:
        ...
        image_pil = Image.open(dataset.images[top_region_index][0]).convert("RGB")
        input_tensor = transform_no_augmentation(image_pil).unsqueeze(0).to(device)
        ...
        masked_tensor = keep_only_top_pixels(input_tensor.squeeze(0), heatmap, num_pixels=50).unsqueeze(0)
