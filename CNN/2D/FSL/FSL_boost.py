...
def cnn_then_boost_classifiersOLD(mem_regions="data_stack"):
    ...
    logger.remove()
    logger.add(f"cnn_APK_boost_classifiers_{mem_regions}.log", format="{message}")
    ...
    model.load_state_dict(torch.load("path/to/pretrained/model", map_location=device))
...
def cnn_then_boost_classifiers(mem_regions="data_stack"):
    root_dir = "path/dataset"
    ...
    for name, clf in boost_models.items():
        ...
        for tag, pred, true in zip(val_apk_tags, preds, val_lbls):
            ...
            apk_labels[apk] = true  # Same label for all images in an APK

        apk_preds = {}
        for apk, votes in apk_votes.items():
            most_common = Counter(votes).most_common(1)[0][0]
            apk_preds[apk] = most_common
...
# Run the function
cnn_then_boost_classifiers("data_stack")
