...
def match_malware_apk_folders(csv_path, malware_dir):
    ...
    malware_val_hashes = df_filtered[df_filtered["Year (Target SDK)"].isin([2022, 2023])]["APK Hash"].tolist()
...
color_mode = "RGB"
benign_test_apks = [benign_apk_paths[i] for i in benign_test_apk_indices if i < len(benign_apk_paths)]
batch_size = 32
model_name = "resnet18"
device = "cuda" if torch.cuda.is_available() else "cpu"
test_benign = CustomDataset(root_dir, transform_no_augmentation, apk_list=benign_test_apks, class_filter=0, selection_mode=mem_regions, color_mode=color_mode)
test_malicious = CustomDataset(root_dir, transform_no_augmentation, apk_list=malicious_test_apks, class_filter=1, selection_mode=mem_regions, color_mode=color_mode)
...
for apk, probs in all_apk_probs.items():
    ...
    pred = int(np.mean(probs) > best_threshold)
    apk_preds[apk] = pred
